#Script will create functions to be used in Shiny app - for hospitalizations
#------------------------------#
#
#------------------------------#
#Libraries
...
plotter_hosp <- function(data, corrected = F, age_groups, color = T){
  ...
  if(color == T){
    ...
    brks <- c(seq(0, max(data[yvar], na.rm=T), by = (max(data[yvar], na.rm=T)+(1000-max(data[yvar], na.rm=T)%%1000))/15))
    p<-ggplot(data,
      ...
      xlab("\nWeek Ending") +
      scale_y_continuous(label=comma, limits=c(0,max(data[yvar], na.rm=T)), breaks= brks) +
      scale_linetype_manual(name = "", values = c("solid", "longdash", "dotted","solid", "longdash", "dotted","solid", "longdash", "dotted","solid")) + 
      scale_colour_manual(name = "Age Group", values = c("0-4 Years" = "#e9cb67", "5-11 Years" = "#e3bb42", "12-17 Years" = "#dbaa2e",
                                                         "18-29 Years" = "#00adad", "30-39 Years" = "#007574", "40-49 Years" = "#004848", "50-64 Years" = "#fcabd5",
                                                         "65-74 Years" = "#f25ead", "75-84 Years" = "#6d1f48", "85+ Years" = "#4C1532")) +
...
plotterpct_hosp <- function(data, corrected = F, age_groups, color = F){
  if("All" %in% age_groups){
    age_groups<-levels(data$age_group)
  }
  data <- data[data$age_group%in%c(age_groups),]
  ...
  if(color == T){
    ...
    p <- ggplot(data,
      ...
      scale_colour_manual(name = "Age Group", values = c("0-4 Years" = "#e9cb67", "5-11 Years" = "#e3bb42", "12-17 Years" = "#dbaa2e",
                                                         "18-29 Years" = "#00adad", "30-39 Years" = "#007574", "40-49 Years" = "#004848", "50-64 Years" = "#fcabd5",
                                                         "65-74 Years" = "#f25ead", "75-84 Years" = "#6d1f48", "85+ Years" = "#4C1532")) +
      scale_x_date(date_breaks = "2 weeks",  limits = c(min(dateVec), max = max(dateVec, na.rm=T)), expand = c(0.01428571,0.01428571)) +
      theme(
        panel.background = element_blank(),
        axis.line = element_line(color = "darkgray"),
        axis.text.x = element_text(angle = 90),
...
plotterrate_hosp <- function(data, age_groups, color = F){
  if("All" %in% age_groups){
    age_groups<-levels(data$age_group)
  }
  data <- data[data$age_group%in%c(age_groups),]
