...
ui <- fluidPage(
    sidebarLayout(
        sidebarPanel(width = 3,
            ...
            hr(),
            radioButtons(inputId = "pctchange", label = "View cases or Pct change plot?", choices = c("Cases" = T, "Pct Change" = F)),
...
server <- function(input, output) {
    ...
    output$plotter_output <- renderPlotly({
        plotter(df_shiny, age_groups = input$age_group_selector, color = input$bwcolor, corrected = input$correctyn)
    })
