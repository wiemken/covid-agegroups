...
cases <-rio::import("https://github.com/wiemken/covid-agegroups/blob/main/mortality%20rates.xlsx?raw=true")
cases <- cases[,-c(12:22)]
names(cases) <- c("week", "a0_4", "a5_11", "a12_15", "a16_17", "a18_29", "a30_39", 
                  "a40_49", "a50_64", "a65_74", "a75")
...
sum(df[1:81,14]) ### total 5-11 deaths

## approx 111 

sum(df[75:81,14]) ## 5-11 in aug/sep
## 12

12/111 # 11 % in past 2 months
## incidence

111 /28368818*100000   ## .39/100,000


### percent 5-11 from 0-18
sum(df[1:81,13]) + sum(df[1:81,14]) + sum(df[1:81,15]) + sum(df[1:81,16])  
111/677

## or 
94 /28368818*100000   ## .39/100,000
